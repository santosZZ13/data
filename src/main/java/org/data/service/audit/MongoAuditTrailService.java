package org.data.service.audit;
...
import org.data.dto.audit.AuditTrailPrimaryKeyDto;
import org.data.util.ClassUtil;
...
public class MongoAuditTrailService {
	...
	public static final List<String> FIELD_IGNORES = List.of(
			"created_by",
			"created_date",
			"updated_by",
			"updated_date",
	...
	public static final List<String> TABLE_IGNORES = List.of(
			"audit_trails",
			"oauth_access_token",
			"oauth_refresh_token"
	);
	...
	public static boolean isAudited(Class<?> clazz) {
		...
		for (String table : tables) {
			if (tableName.equalsIgnoreCase(table) || ALL.equals(tableName)) {
				return Boolean.FALSE;
			}
		}
		return Boolean.TRUE;
	...
	public static AuditTrailDto getCreatedData(Object object) {
		...
		if (isDeleted(object)) table.setAction(String.valueOf(ActionStatus.DELETED));
		else table.setAction(String.valueOf(ActionStatus.CREATED));
		return table;
	}
